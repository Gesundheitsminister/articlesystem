...
if(! function_exists(asrss_str_chop)){
	/**
	...
	*/
	function asrss_str_chop($string, $length = 60, $center = true, $append = null)
	{
		global $mvars;
		if (empty($mvars[9]))
		...
		if ($mvars[9]==1)		
			return as_safe_cr($string,ENT_COMPAT,'UTF-8');	
...
if(! function_exists(asrss_url_creator)){

	function asrss_url_creator($doc,$vars)
	{
		global $_AS,$cfg_client,$sess,$view;
		$url=str_replace('&','&amp;',$doc);
...
if(! function_exists(asrss_element_replacement)){

	function asrss_element_replacement($item_obj,
																	$artsys_obj,
																	$art_images,
																	$art_files,
																	$art_links,
	  ...
	  //Startdate
	  $repl_arr['startdate']=date($cfg_date, $item_obj->convDate2Timestamp(0,'article_startdate'));
	
	  //Starttime
	  if($item_obj->getDataByKey('article_starttime_yn') == 1) {
	    $repl_arr['starttime']=strftime($cfg_time, $item_obj->convDate2Timestamp(0,'article_starttime'));
	  } else {
	    $repl_arr['starttime']='';
	  }
	  ...
	  if($item_obj->getDataByKey('article_enddate_yn') == 1 ) {
	    $repl_arr['enddate']=date($cfg_date, $item_obj->convDate2Timestamp(0,'article_enddate'));
	  } else {
	    $repl_arr['enddate']='';
	  }
	  ...
	  if($item_obj->getDataByKey('article_enddate_yn') == 1 && $item_obj->getDataByKey('article_endtime_yn') == 1) {
	    $repl_arr['endtime']=strftime($cfg_time, $item_obj->convDate2Timestamp(0,'article_endtime'));
	  } else {
	    $repl_arr['endtime']='';
	  }

	  //Titel, Teaser, text & Turnus
	  $repl_arr['title']	=	$item_obj->getDataByKey('title');
	  $repl_arr['teaser']	=	nl2br($item_obj->getDataByKey('teaser'));

		if($artsys_obj->settings['wysiwyg']=='true')
	    $repl_arr['text']	=	$item_obj->getDataByKey('text');
		else
	    $repl_arr['text']	=	nl2br($item_obj->getDataByKey('text'));
	  $repl_arr['turnus_type']	=	$artsys_obj->lang->get('turnus_'.$item_obj->getDataByKey('turnus_type'));
		...
		// old
	      ...
	      $repl_arr['custom'.$i]	=	nl2br($item_obj->getDataByKey('custom'.$i));
				$repl_arr['custom:'.$i]	=	nl2br($item_obj->getDataByKey('custom'.$i));
			} else {
				$repl_arr['custom'.$i]	=	$item_obj->getDataByKey('custom'.$i);
				$repl_arr['custom:'.$i]	=	$item_obj->getDataByKey('custom'.$i);	
			}
	    $repl_arr['custom'.$i.'_label']	=	$artsys_obj->settings['article_custom'.$i.'_label'];
	    $repl_arr['custom_label:'.$i]	=	$artsys_obj->settings['article_custom'.$i.'_label'];
		}
			...
			if (is_array($_AS['temp']['matchesarray'][2]))
				foreach ($_AS['temp']['matchesarray'][2] as $v){
				      ...
				      $repl_arr['image:'.$v]				=	'<img src="'.$art_images['value_txt'][$ek].'" alt="'.$art_images['title'][$ek].'"/>';
				     	$repl_arr['imagethumb:'.$v]		=	'<img src="'.
													        													substr($art_images['value_txt'][$ek],0, strrpos($art_images['value_txt'][$ek],".")).
																														$cfg_client["thumbext"].
																														substr($art_images['value_txt'][$ek],strrpos($art_images['value_txt'][$ek],".")).
			      ...
			      $repl_arr['imageurl:'.$v]					=	$art_images['value_txt'][$ek];
						$repl_arr['imagedesc:'.$v]				=	$art_images['description'][$ek];
						$repl_arr['imagetitle:'.$v]				=	$art_images['title'][$ek];
			  }
				$repl_arr['images']							= '';
	  }
			...
			if (is_array($_AS['temp']['matchesarray'][2]))
				foreach ($_AS['temp']['matchesarray'][2] as $v){
				      ...
				      $repl_arr['file:'.$v]					=	'';
						...
						$repl_arr['filedesc:'.$v]				=	$art_files['description'][$ek];
						$repl_arr['filetitle:'.$v]			=	$art_files['title'][$ek];
			  }
				$repl_arr['files']						 =	'';
	  }
			...
			if (is_array($_AS['temp']['matchesarray'][2]))
				foreach ($_AS['temp']['matchesarray'][2] as $v){
			      ...
			      $repl_arr['linkurl:'.$v]				=	$art_links['value_txt'][$ek];
						$repl_arr['linkdesc:'.$v]				=	$art_links['description'][$ek];
						$repl_arr['linktitle:'.$v]			=	$art_links['title'][$ek];
			  }
				$repl_arr['links']						 =	'';
	  }
	 ...
	 !empty($mvars[4]) ||
	  ...
	  for($iter = $_AS['collection']->get(); $iter->valid(); $iter->next() ) {
	    ...
	    $_AS['temp']['list1_output'] = $mvars[6];
			...
			$_AS['temp']['data1']	=	asrss_element_replacement(	$_AS['item'],
																											$_AS['artsys_obj'],
			...
			if (!empty($_AS['temp']['list2_output']))
				$_AS['temp']['data2']	=	asrss_element_replacement(	$_AS['item'],
																												$_AS['artsys_obj'],
																												$_AS['item_elements']['image'],
																												$_AS['item_elements']['file'],
																												$_AS['item_elements']['link'],
			...
			if (is_array($_AS['temp']['data1']) && $mvars[9]!=2) {
				foreach ($_AS['temp']['data1'] as $k => $v)
					if (empty($mvars[9]))
						...
						$_AS['temp']['data1_conv'][$k]=as_safe_cr(htmlentities($v,ENT_COMPAT,'UTF-8'));
			...
			if (is_array($_AS['temp']['data2']) && $mvars[9]!=2){
				foreach ($_AS['temp']['data2'] as $k => $v) 
					if (empty($mvars[9]))
						...
						$_AS['temp']['data2_conv'][$k]=as_safe_cr(htmlentities($v,ENT_COMPAT,'UTF-8'));
	   	...
	   	$_AS['temp']['result_idcatside']=(int) str_replace('idcatside:','',array_search($_AS['temp']['first_routed_side'][0],$_AS['cat_routing']['routings']));	
			...
			$_AS['temp']['data1']['url'] = asrss_url_creator( $cfg_client['htmlpath'].$con_side[$_AS['temp']['result_idcatside']]['link'],
	    ...
	    //Hinzufügen, wenn nur aktuelle gew?nscht oder alle
				...
				foreach ($_AS['temp']['data1'] as $k => $v){
					...
					// global if-statement
					if(strpos($_AS['temp']['list1_output'],'{if_'.$k.'}')!==false){	
						if (empty($v))
						  $_AS['temp']['list1_output'] = preg_replace('#\{if_'.$k.'\}(.*)\{/if_'.$k.'\}#sU','',$_AS['temp']['list1_output']);
						else
						  $_AS['temp']['list1_output'] = str_replace(array('{if_'.$k.'}','{/if_'.$k.'}'), array('',''), $_AS['temp']['list1_output']);
					}
					// global if-not-statement
					if(strpos($_AS['temp']['list1_output'],'{if_not_'.$k.'}')!==false)
						if (empty($v))
					  	$_AS['temp']['list1_output'] = str_replace(array('{if_not_'.$k.'}','{/if_not_'.$k.'}'), array('',''), $_AS['temp']['list1_output']);
						else
						 	$_AS['temp']['list1_output'] = preg_replace('#\{if_not_'.$k.'\}(.*)\{/if_not_'.$k.'\}#sU','',$_AS['temp']['list1_output']);
				...
				foreach ($_AS['temp']['data2'] as $k => $v){
					...
					// global if-statement
					if(strpos($_AS['temp']['list2_output'],'{if_'.$k.'}')!==false){	
						if (empty($v))
						  $_AS['temp']['list2_output'] = preg_replace('#\{if_'.$k.'\}(.*)\{/if_'.$k.'\}#sU','',$_AS['temp']['list2_output']);
						else
						  $_AS['temp']['list2_output'] = str_replace(array('{if_'.$k.'}','{/if_'.$k.'}'), array('',''), $_AS['temp']['list2_output']);
					}
					// global if-not-statement
					if(strpos($_AS['temp']['list2_output'],'{if_not_'.$k.'}')!==false)
						if (empty($v))
					  	$_AS['temp']['list2_output'] = str_replace(array('{if_not_'.$k.'}','{/if_not_'.$k.'}'), array('',''), $_AS['temp']['list2_output']);
						else
						 	$_AS['temp']['list2_output'] = preg_replace('#\{if_not_'.$k.'\}(.*)\{/if_not_'.$k.'\}#sU','',$_AS['temp']['list2_output']);
	  ...
	  $_AS['output']['list_body'] = str_replace( '{content2}',$_AS['temp']['rows2'],$_AS['output']['list_body']);
			...
			if (!empty($_AS['temp']['chopparts']))
		  	foreach ($_AS['temp']['chopparts'][1] as $k => $v)
		  		$_AS['output']['list_body']=str_replace(	$_AS['temp']['chopparts'][0][$k],
		  																				asrss_str_chop($v, $mvars['1003'], $mvars['1004'], $mvars['1005']),
		...
		@fwrite ($_AS['handle'],'<'.'?xml version="1.0" encoding="utf-8"?'.'>'.$_AS['output']['list_body']);
		fclose ($_AS['handle']); 
...
if(! function_exists(as_safe_cr)){
	function as_safe_cr($string,$mode=''){
	 $cr = array(
